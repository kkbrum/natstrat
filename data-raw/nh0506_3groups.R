## Code to prepare `nh0506_3groups` dataset
...
z <- rep(1, length(neversmoker))
z[!is.na(dailysmoker) & dailysmoker] <- 2
z[!is.na(neversmoker) & neversmoker] <- 0
z <- factor(z, levels = 0:2, labels = c("never smoker", "some smoking", "daily smoker"))

ds <- data.frame(SEQN, z, sex, age, race, education, povertyr, bmi,
                 cigsperday30, cotinine, homocysteine)
nh0506_3groups <- ds[age >= 20 & !is.na(z) & !is.na(homocysteine) &
               !is.na(cotinine), ]
# As there are only five people remaining with unknown education at this step, remove them
nh0506_3groups <- nh0506_3groups[nh0506_3groups$education != "Unknown education", ]
nh0506_3groups$education <- factor(nh0506_3groups$education, levels = levels(nh0506_3groups$education),
...
usethis::use_data(nh0506_3groups, overwrite = TRUE)
